...
[CmdletBinding(DefaultParameterSetName='Items', HelpUri='https://go.microsoft.com/fwlink/?LinkID=2096492')]
    ...
    if ($ifPipeline) {
        ...
        foreach ($item in $items) {
            ...
            if ($i -eq 0 -and $HideHeader -and -not $RemoveTopBlankLine) {###########
                    Write-Host ""
            }
...
function Out-ChildItemColor {
    ...
    begin {
        try {
            ...
            if($PSBoundParameters.ContainsKey('HideHeader')) {
                $PSBoundParameters.Remove('HideHeader') | Out-Null
            }
            for ($l=1; $l -lt $GetChildItemColorVerticalSpace; $l++) {
                Write-Host ""
            }
    ...
    process {
        try {
            if (($_ -is [System.IO.DirectoryInfo]) -or ($_ -is [System.IO.FileInfo])) {
                FileInfo $_ -HumanReadableSize:$HumanReadableSize -HideHeader:$HideHeader
                $_ = $Null
            }
...
process
{
    try {
        ...
        if ($ifPipeline) {
            ...
            $items | Out-ChildItemColor -HumanReadableSize:$HumanReadableSize -HideHeader:$HideHeader
        }
    } catch {
        throw
    }
